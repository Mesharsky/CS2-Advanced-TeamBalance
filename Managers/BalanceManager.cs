...
namespace AdvancedTeamBalance
{
    ...
    public static class BalanceManager
    {
        ...
        /// <summary>
        /// Public method to get player value for external use
        /// </summary>
        public static double GetPlayerValuePublic(Player player, string balanceMode)
        {
            return GetPlayerValue(player, balanceMode);
        }
                ...
                if (sizeDiff <= maxTeamSizeDifference)
                {
                    ...
                    if (_config.Balancing.BoostAfterLoseStreak > 0)
                    {
                        if (ctWinStreak >= _config.Balancing.BoostAfterLoseStreak) // T team is losing
                        {
                            ...
                            boostPercentageToUse = GetProgressiveBoostPercentage(ctWinStreak);
                            boostMultiplier = 1.0 + (boostPercentageToUse / 100.0);
                            if (logDetails && _config.General.EnableDebug) 
                            {
                                Console.WriteLine($"[AdvancedTeamBalance] T team losing streak ({ctWinStreak}), boosting T skill by {boostPercentageToUse}%");
                                if (_config.Balancing.ProgressiveBoost)
                                    Console.WriteLine($"[AdvancedTeamBalance] Progressive boost active - using tier-based percentage: {boostPercentageToUse}%");
                            }
                        }
                        else if (tWinStreak >= _config.Balancing.BoostAfterLoseStreak) // CT team is losing
                        {
                            losingTeam = CsTeam.CounterTerrorist;
                            boostPercentageToUse = GetProgressiveBoostPercentage(tWinStreak);
                            boostMultiplier = 1.0 + (boostPercentageToUse / 100.0);
                            if (logDetails && _config.General.EnableDebug) 
                            {
                                Console.WriteLine($"[AdvancedTeamBalance] CT team losing streak ({tWinStreak}), boosting CT skill by {boostPercentageToUse}%");
                                if (_config.Balancing.ProgressiveBoost)
                                    Console.WriteLine($"[AdvancedTeamBalance] Progressive boost active - using tier-based percentage: {boostPercentageToUse}%");
                            }
                        }
                        
                        // Show boost application message if configured
                        if (losingTeam != CsTeam.None && _config.Balancing.ShowBoostOnApplication && 
                            _config.Messages.AnnounceBalancing && _config.Messages.ExplainBalanceReason)
                        {
                            string teamName = losingTeam == CsTeam.Terrorist ? "Terrorists" : "Counter-Terrorists";
                            int loseStreak = losingTeam == CsTeam.Terrorist ? ctWinStreak : tWinStreak;
                            ChatHelper.PrintLocalizedChatAll(true, "balance.boost.applying", teamName, loseStreak, boostPercentageToUse);
                        }
                    }
        ...
        /// <summary>
        /// Get the boost percentage based on progressive boost settings
        /// </summary>
        private static int GetProgressiveBoostPercentage(int loseStreak)
        {
            if (!_config.Balancing.ProgressiveBoost || _config.Balancing.BoostTiers == null || _config.Balancing.BoostTiers.Count == 0)
            {
                return _config.Balancing.BoostPercentage;
            }
            
            // Find the highest tier that the lose streak qualifies for
            int boostPercentage = 0;
            foreach (var tier in _config.Balancing.BoostTiers.OrderBy(t => t.Key))
            {
                if (loseStreak >= tier.Key)
                {
                    boostPercentage = tier.Value;
                }
            }
            
            // If no tier matched or boost is 0, use default
            return boostPercentage > 0 ? boostPercentage : _config.Balancing.BoostPercentage;
        }
