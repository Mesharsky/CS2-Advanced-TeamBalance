...
namespace AdvancedTeamBalance
{
    ...
    public static class EventManager
    {
        ...
        /// </summary>
        private static void PerformTeamBalancing(bool forceBalance = false)
        {
            ...
            // Otherwise perform normal balancing - pass win streaks for boosting losing team
            ...
            if (_config.Messages.AnnounceBalancing && (result.PlayersMoved > 0 || result.SwapsMade > 0))
            {
            ...
            if (forceBalance)
            {
                ...
                tPlayers = PlayerManager.GetPlayersByTeam(CsTeam.Terrorist);
                ctPlayers = PlayerManager.GetPlayersByTeam(CsTeam.CounterTerrorist);
                
                var forcedResult = BalanceManager.ForceTeamSizeBalance(tPlayers, ctPlayers, _config.General.EnableDebug);
                
                if (forcedResult > 0 && _config.Messages.AnnounceBalancing)
                {
                    ChatHelper.PrintLocalizedChatAll(true, "balance.force", forcedResult);
                }
